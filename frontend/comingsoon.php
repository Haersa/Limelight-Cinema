...
<head>
    ...
    <title>Limelight Cinema - Coming Soon</title>
...
<nav class="navbar"> <!--start of navbar -->
    <div class="container"> <!-- start of navbar container -->
        ...
        <div class="nav-links">
            ...
            <a href="comingsoon.php" class = "nav-active">Coming Soon</a> <!--page links -->
...
<nav class="mobile-nav"> <!-- start of mobile menu navigation -->
    ...
    <a href="movies.php">Movies</a> <!-- page links -->
    <a href="cinemas.php">Cinemas</a> <!-- page links -->
    <a href="comingsoon.php" class = "mobile-nav-active">Coming Soon</a> <!-- page links -->
    <a href="specialoffers.php">Special Offers</a> <!-- page links -->
...
<div class="slider-wrapper desktop-slider"> <!-- slider for bigger screens -->
    <div class="slider-container">
        <div class="slider"> <!-- only show coming soon movies since WHERE = Coming_Soon = 'Yes'  -->
            <?php
            $sql = "SELECT FilmID, FilmTitle, poster_img, Cast, RunTime
                     FROM LimelightFilms
                     WHERE Coming_Soon = 'Yes' 
                     LIMIT 5";
            $result = $conn->query($sql);
            
            while($row = $result->fetch_assoc()) {
                echo '<div class="slide">';
                echo '<img src="' . $row['poster_img'] . '" alt="' . $row['FilmTitle'] . '">';
                echo '<div class="slide-content">';
                echo '<div class="slide-info">';
                echo '<h2>' . $row['FilmTitle'] . '</h2>';
                echo '<p class="cast">Cast: ' . $row['Cast'] . '</p>';
                echo '</div>';
                echo '<div class="run-time">';
                echo '<i class="fas fa-clock"></i> ' . $row['RunTime'] . ' mins';
                echo '</div>';
                echo '</div>';
                echo '</div>';
            }
...
<div class="mobile-slider"> <!-- slider for mobile screens -->
    <?php
    $sql = "SELECT FilmID, FilmTitle, poster_img
             FROM LimelightFilms
             WHERE Coming_Soon = 'Yes'
             LIMIT 5";
    $result = $conn->query($sql);
    ...
    while($row = $result->fetch_assoc()) {
        echo '<div class="mobile-slide">';
        echo '<img src="' . $row['poster_img'] . '" alt="' . $row['FilmTitle'] . '">';
        echo '<div class="mobile-slide-content">';
        echo '<h2>' . $row['FilmTitle'] . '</h2>';
        echo '</div>';
        echo '</div>';
    }
